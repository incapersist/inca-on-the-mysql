/*------------------------------------------------------
  ...
  -------------------------
   ...
   void main(int argc, char **argv)
   {
      ...
      declaredStringType p1_1, p1_2, p2_1;
      try
      {  
   ...
   If this class is used in an MFC application, declaredStringType is CString, else
...
#define declaredStringType CString
#else
// if we're not using MFC, use STL strings
#define declaredStringType std::string
#endif
...
struct CCmdParam
{
   std::vector<declaredStringType> m_strings;
};

// this class is actually a map of strings to vectors
typedef std::map<declaredStringType, CCmdParam> _CCmdLine;
...
public:
   ...
   /*------------------------------------------------------
      ...
      declaredStringType CCmdLine::GetSafeArgument(const char *pSwitch, int iIdx, const char *pDefault)
   ...
   declaredStringType  GetSafeArgument(const char *pSwitch, long unsigned int iIdx, const char *pDefault);

   /*------------------------------------------------------

      declaredStringType CCmdLine::GetArgument(const char *pSwitch, int iIdx)
   ...
   ------------------------------------------------------*/
   declaredStringType  GetArgument(const char *pSwitch, long unsigned int iIdx); 
