...
class Date
{
	public:
        ...
        //
		enum format_type {MDY, DAY, MONTH, FULL, EUROPEAN, COLLATE, DMY, DMY2, DMYTIME};
        enum {OFF, ON};
        enum {BUF_SIZE=40};
	...
	protected:
		double julian;	// see julDate();  days since 1/1/4713 B.C.
	...
	private:
		...
		int		isLeapYear(int)	const;		// returns true if leap year, false if not

	public:
		Date ();
		Date (short m, short d, short y, short h = 0, short mn = 0, short s = 0);
		Date (long 	j);
//		Date (const char 	*dat);
		Date (const Date 	&dt);
		Date (const tm	 	&TM);
...
*/
		...
		friend std::istream &operator >> (std::istream &is, const Date &dt);
/*
#if defined(MSDOS) || defined(_WIN32)
...
inline double	julDate()		const	{ return(julian); }	// returns julian date
        ...
        void decodeTime(int& h, int& m, int& s) { h = hour; m = minute; s = sec; }
