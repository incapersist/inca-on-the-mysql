...
// This was only used by the original leap year function, which was broken
...
Date::Date (short m, short d, short y, short h, short mn, short s) : month(m), day(d), year(y), hour(h), minute(mn), sec(s)
{
	assert(hour >= 0);
	assert(hour < 24);
...
Date Date::operator --(int)
{
		Date temp=*this;				// TML - Necessary to save current
										// value of (*this) in order to
		julian -= 1;						// simulate postfix operation!
...
{
	day_of_week = (enum Wday) (((long)julian + 2) % 7 + 1);
}
...
{
	long z = (long)julian+1;

	double seconds = (julian - z + 1) * 86400;
	hour = short(seconds / 3600);
	minute = short((seconds - hour * 3600) / 60);
	sec = short(seconds - hour * 3600 - minute * 60);

    long a = (long) julian + 32044 + 1;
    long b = (4*a+3)/146097;
    long c = a - (b*146097)/4;
...
void Date::julian_to_mdy ()
{
	...
	double seconds = (julian - z + 1) * 86400;
	hour = short(seconds / 3600);
	min = short((seconds - hour * 3600) / 60);
	sec = short(seconds - hour * 3600 - min * 60);
...
void Date::mdy_to_julian (void)
{
	...
	double timefraction = (hour * 3600 + minute * 60 + sec) / 86400.0;
	julian = j + timefraction;
...
void Date::mdy_to_julian (void)
{
	...
	double timefraction = (hour*3600 + min*60 + sec) / 86400.0;
	julian = j + timefraction;
	...
	switch ( type )
	{
		...
		case Date::DMY:
			...
			else
			{
				...
				sprintf( buf+strlen(buf), "%1d/%1d/%02d", day, month, y);
			}
			break;
		...
		case Date::DMYTIME:
			...
			else
			{
				...
				sprintf( buf+strlen(buf), "%02d/%02d/%04d %02d:%02d:%02d", day, month, y, hour, minute, sec);
			}
			break;
		...
		default:
			...
			else
			{
				...
				sprintf( buf+strlen(buf), "%02d/%02d/%04d", day, month, y);
			}
			break;
	}
...
int Date::isLeapYear( int testYear ) const
{
	return  ( (testYear >= 1582) ?
			  ( (testYear % 4 == 0  &&  testYear % 100 != 0)  ||  testYear % 400 == 0 ):
			  (testYear % 4 == 0) );
}
