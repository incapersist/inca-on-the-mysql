/**
 * The <code>DbAbsSeries</code> fetches model driving data from a MySQL database, via a supplied session.
 *
...
public:
	...
	DbAbsSeries(std::shared_ptr<mysqlx::Session>& session, const std::string& tableName);
	~DbAbsSeries();
	 ...
	 */
	std::vector<CAbsSeries*> fetch(DbModelIndexers& modelIndexers, DbModelRun& dbModelRun);
...
private:
	 ...
	 * @param[in]		dbModelRun is the model run meta data
	 */
	void fetchReachData(CAbsSeries& abs, DbModelIndex& reach, unsigned int inputFileId, DbModelRun& dbModelRun);
};
