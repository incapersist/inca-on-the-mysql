/**
 * The <code>DbDepSeries</code> fetches model driving data from a MySQL database, via a supplied session.
 *
...
public:
	...
	DbDepSeries(std::shared_ptr<mysqlx::Session>& session, const std::string& tableName);
	~DbDepSeries();
	 ...
	 */
	CDepSeries* fetch(DbModelIndexers& modelIndexers, DbModelRun& dbModelRun);
...
private:
	 ...
	 * @param[in]		dbModelRun is the model run meta data
	 */
	void fetchReachData(CDepSeries& dep, DbModelIndex& reach, unsigned int inputFileId, DbModelRun& dbModelRun);
};
