/**
 * The <code>DbFertSeries</code> fetches model driving data from a MySQL database, via a supplied session.
 *
...
public:
	...
	DbFertSeries(std::shared_ptr<mysqlx::Session>& session, const std::string& tableName);
	~DbFertSeries();
	 ...
	 * @param[in]	fileId is the input fileId for which data will be retrieved
	 ...
	 */
	CFertSeries* fetch(DbModelIndexers& modelIndexers, DbModelRun& dbModelRun, unsigned int fileId);
...
private:
	...
	unsigned int getLandIndexOffset(unsigned int parameterSetId);
	 ...
	 * @throw		A simple, generic exception if anything goes wrong with the retrieval
	 */
	CBaseSeries* fetchData(unsigned int reachId, unsigned int inputFileId, DbModelRun& dbModelRun, unsigned int landCount, unsigned int landIndexOffset);
	 ...
	 * @param[in]		dbModelRun is the model run meta data
	 */
	void fetchReachData(CFertSeries& fert, DbModelIndex& reach, unsigned int inputFileId, DbModelRun& dbModelRun, unsigned int landCount, unsigned int landIndexOffset);
};
